...
def _rolling_pct_rank(
    values: np.ndarray, window: int, min_periods: int = 30
) -> np.ndarray:
...
def build_fred_macro(
    ...
    for idx, (id, col_name) in enumerate(_FRED.items()):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                s = fred.get_series(
                    series_id=id, observation_start=start, observation_end=end_date
                )
                data_acquired = True
                break
                ...
                if attempt == max_retries:
                    ...
                    wait = base_delay
                    print(
                        f"[WARN] {id} attempt {attempt}/{max_retries}: {e}  (retrying in {wait:.1f}s)"
                    )
                    time.sleep(wait)
        ...
        df = (
            pl.DataFrame(
                {
                    "Date": pl.Series([_as_date(d) for d in s.index], dtype=pl.Date),
    ...
    macro = _add_pct_rank(
        macro, "vix", window=365, min_periods=50, output_col="vix_pct_rank_1y"
    )
    ...
    macro = (
        ...
        .with_columns([(pl.col("Date") + pl.duration(days=1)).alias("Join_date")])
        .drop("Date")
        .sort("Join_date")
    )
    ...
    def _vintage_set(series_id: str) -> Set[date]:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt == max_retries:
                    print(
                        f"  [fail] {series_id}: {exc}  (all {max_retries} attempts exhausted)"
                    )
                    return set()
                wait = base_delay
                print(
                    f"  [warn] {series_id} attempt {attempt}/{max_retries}: {exc}  (retrying in {wait:.1f}s)"
                )
                time.sleep(wait)
