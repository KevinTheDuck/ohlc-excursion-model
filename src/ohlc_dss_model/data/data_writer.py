...
def write_parquet(
    df: pl.DataFrame, name: str, file_path: str = config.data.processed_folder_path
) -> None:
    try:
...
def write_csv(
    df: pl.DataFrame,
    name: str,
    file_path: str = config.data.processed_folder_path,
    separator: str = config.data.csv_separator,
) -> None:
    try:
